...
<header>
    <h2>STAJI BITENLERI GORUNTULE</h2>
</header>
...
<section>
    ...
    <%
        ...
        resultSet = statement.executeQuery("select min_kabul_edilen_gun from ayarlar;");
        while (resultSet.next()){
            min_kabul_edilen_gun = resultSet.getInt("min_kabul_edilen_gun");
        }
    ...
    <table border=1>
        ...
        try{
            connection = DriverManager.getConnection(connectionUrl+database, userid, password);
            statement=connection.createStatement();
            String sql ="select ogrenci_no, isim, soyisim, ogretim, toplam_staj_gunu from normal_ogrenci where toplam_staj_gunu>=" +min_kabul_edilen_gun +";";
    ...
    %>
        <%
            session.setAttribute("min_kabul_edilen_gun",min_kabul_edilen_gun);
        %>
        <%
            System.out.println(min_kabul_edilen_gun +"--stgb");
        %>
    </table>
    <br>
    <form action="excel_staj_biten.jsp">
        <button class="button button2" type="submit"> EXCELE AKTAR </button>
    </form>
</section>
